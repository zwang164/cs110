...
class Character(pygame.sprite.Sprite):
	...
	#laser
		#laserImage = sprtSheet.SpriteSheet("11.png")
		#self.laserClip =[]
		#self.laserClipR = laserImage.getImage(0, 150, 800, 100)
		#self.laserClipL = pygame.transform.flip(self.laserClipR,True,False)
	...
	def fight(self, fight, opponent):
		...
		if(self.invincibility == False):
			...
			if(fight == "punch"):
				print(self.frames[0].get_rect())
				print(self.image.get_rect())
				print(opponent.frames[0].get_rect())
				print(opponent.image.get_rect())
	...
	def ultimate(self,opponent,screen):
		if(self.energyRate == 100):
			...
			if(self.facing == "left"):
				...
				self.frames = screen.blit(self.laserClipL,(0,0))
				print(self.rect.x,self.rect.y)
				print(opponent.rect.x,opponent.rect.y)
				pygame.time.delay(1000)
				if(self.frames.colliderect(opponent.rect)):
					opponent.health -= 200
			...
			if(self.facing == "right"):
				print(self.rect.x,self.rect.y)
				print(opponent.rect.x,opponent.rect.y)
				self.bs = screen.blit(self.laserClipR,(self.rect.x,self.rect.y))   #self.sprite.draw(self.screen) from main cause error
				pygame.time.delay(1000)
				if(self.bs.colliderect(opponent.rect)):
					opponent.health -= 200
					if(opponent.rect.x < 1280-370):     #not outside of screen
						opponent.rect.x += 50
				pygame.display.flip()
			'''
			self.savePos = self.rect.x
			if(self.facing == "left"):
				newPos = self.rect.x - 2223 + 350   #took longer to load image so sometimes it does not appear only press once
				self.rect.x = newPos    #since the newPos(end of laser) is outside of screen, the collision will not be working
				...
				print(opponent.frames[0].get_rect())
				print(opponent.image.get_rect())
				objects = self.frames[0].get_rect()
				if (objects.colliderect(opponent.image.get_rect()) and self.facing != opponent.facing):   #test if two rect overlap instead; rect is too big so will always collide so distinguish by facing
					print(2)
					self.hit(opponent, 200, 50)
				## If you miss this giant laser that's your fault
				#if (pygame.sprite.collide_rect(self.frames[0].get_rect(), opponent)):   #left.rect.colliderect(right.rect)
				#if (pygame.sprite.collide_rect(opponent,self)):   #left.rect.colliderect(right.rect)
					#print(1)
					#self.hit(opponent, 300, 500)
			if(self.facing == "right"):
				...
				print(opponent.image.get_rect())
				objects = self.frames[0].get_rect()
				if(objects.colliderect(opponent.image.get_rect()) and self.facing != opponent.facing):   #test if two rect overlap instead
					print(1)
					self.hit(opponent, 200, 50)
